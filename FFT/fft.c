...
volatile int syll_cnt = 0;
volatile int chirp_cnt = 0;
...
static PT_THREAD (protothread_fft(struct pt *pt))
{
    ...
    while(1) {
        ...
        //printf("Max Frequency (Hz): %f\n",max_freqency);
        if (abs(max_freqency - 2300.0)<50){
            counting_chirps();
        }
        else{
            cnt_1 += 1;
